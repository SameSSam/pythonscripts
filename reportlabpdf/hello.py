...
def bezier2(canvas):
    ...
    for xoffset in (1,-1):
        yoffset = xoffset
        for (dx,dy) in dxdy:  #每次形成8个坐标点,整个循环16个坐标点
            px = xc + xd*xoffset*dx
            py = yc + yd*yoffset*dy
            pointlist.append((px,py))
        yoffset = -xoffset
        for (dy,dx) in dxdy: #每次形成8个坐标点，整个循环16个坐标点(x,y的值刚好相反)
            px = xc+xd*xoffset*dx
            py = yc+yd*yoffset*dy
            pointlist.append((px,py))
    ...
    while pointlist:
        ...
        canvas.setLineWidth(inch*0.1)
        canvas.setStrokeColor(green)
        canvas.line(x1,y1,x2,y2)  #画出起始段切线
        canvas.setStrokeColor(red)
        canvas.line(x3,y3,x4,y4)  #画出末端切线
        #finally draw the curve,画出曲线
        canvas.setStrokeColor(black)
        canvas.bezier(x1,y1,x2,y2,x3,y3,x4,y4)
