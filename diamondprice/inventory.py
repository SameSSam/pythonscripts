...
def get_inventory_df():
    cols = ['条码号', '石料名称','规格', '重量', '形状', '颜色', '净度', '切工', '抛光', '对称', '荧光', '证书类型', '证书号']
    df_igi = pd.read_excel(DATA_DICT['INVENTORY_FILE_IGI'],header=1)
    df_gia = pd.read_excel(DATA_DICT['INVENTORY_FILE_GIA'],header=0, usecols= cols)
    df_gia.rename(columns={'条码号':'货号','石料名称':'培育','规格':'分数','证书类型':'证书'},inplace=True)
    df_gia['分数'] = df_gia['分数'].map(lambda x: x[:-2]).astype(float)

    #合并两个df
    df_inventory = pd.concat([df_igi,df_gia],ignore_index=True,axis = 0)
    return df_inventory
